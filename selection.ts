...
export const getVisibleAndNonSelectedElements = <T extends NonDeletedExcalidrawElement>(
  elements: readonly T[],
    ...
    selectedElements.forEach((element) => {
      if (isFrameLikeElement(element)) {
        getFrameChildren(elements, element.id).forEach(
          (e) =>
            !addedElements.has(e.id) && elementsToInclude.push(e as NonDeletedExcalidrawElement),
        )
      }
